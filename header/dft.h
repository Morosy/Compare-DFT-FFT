...
*/
COMPLEX *iDFT_COMPLEX(COMPLEX *, int);
...
COMPLEX *iDFT_COMPLEX(COMPLEX *FUNCTION, int N)
{
    COMPLEX *function = (COMPLEX *)malloc(sizeof(COMPLEX) * N);
    for(int n = 0; n < N; n++){
        function[n].real = 0.0;
        function[n].imaginary = 0.0;
        for(int k = 0; k < N; k++){
            COMPLEX euler = EULER((2 * M_PI * k * n) / N);
            COMPLEX temp;
            temp = MUL(FUNCTION[k], euler);
            function[n].real += temp.real;
            function[n].imaginary += temp.imaginary;
        }
        function[n].real /= N;
        function[n].imaginary /= N;
    }
    return function;
}
