...
/*
    ...
    @return フィルタ後の信号
*/
COMPLEX* lowpass_Filter(COMPLEX *data, int cutoff, int N);

/*
    @brief 指定周波数範囲内の振幅を2倍にする
    @param COMPLEX型の信号
    @param 周波数
    @param High(= 0): Highにフィルタ適用 or Low(= 1): Lowにフィルタ適用
    @param 配列の要素数
    @return 指定周波数範囲内の振幅を2倍にした信号
*/
COMPLEX *amplitude_Doubled_Filter(COMPLEX *, int, int, int);


/*
    @brief 指定周波数範囲内の最大振幅を返す
    @param COMPLEX型の信号
    @param 開始周波数
    @param 終了周波数
    @param 配列の要素数
    @return 指定周波数範囲内の最大振幅
*/
double return_Max_Power_In_Range(COMPLEX *, int, int, int);
...
/*
    ...
    @return 最大振幅を持つ周波数
*/
double return_Max_Frequency(COMPLEX *, int, double);
...
/*
    ...
    @return 最小振幅を持つ周波数
*/
double return_Min_Frequency(COMPLEX *, int, double);
...
double return_Max_Frequency(COMPLEX *data, int N, double sampling_rate)
{
    // 最大振幅を持つ周波数成分を見つける
    int max_index = 0;
    double max_power = 0;
...
double return_Min_Frequency(COMPLEX *data, int N, double sampling_rate)
{
    // 最小振幅を持つ周波数成分を見つける
    int min_index = 0;
    double min_power = 0;
    ...
    // 周波数を計算
    double frequency = (double)min_index * sampling_rate / N;
    return frequency;
}
...
COMPLEX *amplitude_Doubled_Filter(COMPLEX *data, int cutoff, int HorL, int N)
{
    COMPLEX *output = (COMPLEX *)malloc(sizeof(COMPLEX) * N);
    init_complex(output, N);
    for(int k = 0; k < N; k++){
        if(HorL == 1){
            if(k < cutoff){
                ...
                output[k].real = data[k].real * 2;
                output[k].imaginary = data[k].imaginary * 2;
            }
        }else if(HorL == 0){
            if(k < cutoff){
                output[k].real = data[k].real * 2;
